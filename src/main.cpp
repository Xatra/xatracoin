...
const string strMessageMagic = "XatraCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nTotalHeight, int64_t nFees)
{
...
const int SIX_MONTHS_BLOCK_COUNT = 262800;
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
	int64_t nRewardCoinYear = COIN_YEAR_REWARD; //10% reward up to end
	if(nBestHeight < SIX_MONTHS_BLOCK_COUNT)
		nRewardCoinYear = 5 * COIN_YEAR_REWARD; //50% reward for 131400 blocks
	else if(nBestHeight < (2 * SIX_MONTHS_BLOCK_COUNT))
		nRewardCoinYear = 4 * COIN_YEAR_REWARD; //40% reward for 131400 * 2 blocks
	else if(nBestHeight < (3 * SIX_MONTHS_BLOCK_COUNT))
		nRewardCoinYear = 3 * COIN_YEAR_REWARD; //30% reward for 131400 * 3 blocks
	else if(nBestHeight < (4 * SIX_MONTHS_BLOCK_COUNT))
		nRewardCoinYear = 2 * COIN_YEAR_REWARD; //20% reward for 131400 * 4 blocks
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "XatraCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "On 30 March 2017 SpaceX sent one of its used Falcon 9s first time in history";
        CTransaction txNew;
        txNew.nTime = 1519729974;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1519729980;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
		block.nNonce   = !fTestNet ? 381467 : 1;
        // genesis block miner
...
unsigned char pchMessageStart[4] = { 0x71, 0xEF, 0xF9, 0xBD };
